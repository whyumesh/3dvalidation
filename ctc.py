...
def create_division_excel_report(div_code, affiliate, div_name, summary_df, detail_df, output_dir):
    ...
    try:
        ...
        if total_row is None:
            total_row = header_row + 50  # Default position with space for ZBM rows
        ...
        for col_idx in range(1, min(30, ws.max_column + 1)):
            ...
            if header_val:
                ...
                # Map columns based on template (ZBM columns first)
        ...
        # Process and write ZBM-level data rows
        ...
        if detail_df is not None and not detail_df.empty:
            ...
            # Add all metric columns that exist in detail_df
            metric_cols = [
                '# Unique TBMs',
                '# Unique HCPs',
                '# Requests Raised\n(A+B+C)',
        ...
        for col_name, col_idx in column_mapping.items():
            # Skip ZBM-specific columns in Total row
